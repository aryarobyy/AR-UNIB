...
class BuildingEdit extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState createState() => _BuildingEditState();
}

class _BuildingEditState extends ConsumerState<BuildingEdit> {
  ...
  void _initializeFields() {
    ...
    _imageUrlController.text = building.imageUrl ?? '';

    if (building.location != null) {
      _latitudeController.text = building.location!.latitude.toString();
      _longitudeController.text = building.location!.longitude.toString();
    }
  }
    ...
    return Container(
      ...
      child: _selectedImage != null
        ...
        child: Image.file(
          ...
          : (widget.building?.imageUrl?.isNotEmpty == true)
          ? ClipRRect(
        borderRadius: BorderRadius.circular(15),
        child: Image.network(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Apakah Anda yakin ingin menghapus "${widget.building!.name}"? '
  ...
  Future<void> _deleteBuilding() async {
    ...
    try {
      await notifier.deleteBuilding(widget.building.id!);

      mySnackbar(context, 'Gedung berhasil dihapus', Type.success);
      Navigator.of(context).pop(true);
